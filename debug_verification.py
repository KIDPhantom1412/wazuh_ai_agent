    ...
    for strategy_name, output in strategies:
        ...
        try:
            ...
            if matched_id_int in expected_rule_ids:
                ...
                print(
                    f"  ❌ Matched ID {matched_id_int} NOT in expected IDs {sorted(expected_rule_ids)}"
                )
    ...
    for idx, sample_log in enumerate(sample_logs):
        ...
        if isinstance(sample_log, dict):
            ...
            for field in fields_to_try:
                ...
                if value:
                    print(
                        f"  ✅ Found '{field}': {value[:100]}..."
                        if len(str(value)) > 100
                        else f"  ✅ Found '{field}': {value}"
                    )
                else:
...
if __name__ == "__main__":
    ...
    test_response_1 = {
        "data": {"output": {"rule": {"id": "110001", "level": 5, "description": "Test rule"}}}
    }
    ...
    test_response_2 = {"data": {"rule": {"id": 110001, "level": 5}}}  # Note: int instead of string
    ...
    sample_logs = [
        ...
        {"message": "Authentication failure", "data": {"log": "Failed login attempt"}},
