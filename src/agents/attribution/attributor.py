...
def get_attributor_agent(model: BaseChatModel, indexer_agent):
    ...
    workflow.add_node(
        "investigation", partial(investigation_node, model=model, indexer_agent=indexer_agent)
    )
    workflow.add_node("report_generation", partial(report_generation_node, model=model))
